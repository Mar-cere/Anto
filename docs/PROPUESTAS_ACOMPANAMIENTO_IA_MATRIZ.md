...
Listado de **~181** propuestas `No` en la tabla principal, sin filas **`Parcial`** abiertas — **smoke dispositivo físico** pendiente en crisis **#10**, **#93**, **#205** y validación nativa EN en **#151** (spec [PROTOCOLO_CRISIS_V1.md](./PROTOCOLO_CRISIS_V1.md); releases **1.5.0** / **1.5.1** / **1.5.2**). **#9** derivación humana **pospuesta** (Q4). **#2** focos → **Sí*** (jul 2026). **11 anclas entregadas** referenciadas (numeración **#1–#223**; huecos históricos conservados; **16** ex-Q5 + **11** entregadas archivadas). **Hecha:** `Sí` / `Sí*` / `Parcial` = ver columna. Chat → tarea/hábito: [CONTRATO_CHAT_ACCIONES_V1.md](./CONTRATO_CHAT_ACCIONES_V1.md). Compromisos entre sesiones: [CONTRATO_COMPROMISOS_V1.md](./CONTRATO_COMPROMISOS_V1.md) (**#20**, **#202**). Crisis → [PROTOCOLO_CRISIS_V1.md](./PROTOCOLO_CRISIS_V1.md).
...
### Estado del código (revisión 14 jul 2026)
...
| **Focos de acompañamiento (#2)** | **Sí* (implementado, pendiente release):** 3 fases completadas — (1) Backend: catálogo 3 temas (ansiedad, límites, autocuidado), API REST `/api/focus/*`, subdocumento `activeFocus` en User, `focusService` con transiciones de estado, validación Joi, rate limiting, i18n ES/EN, hardening edge cases. (2) Frontend: `FocusOnboardingScreen`, `FocusProgressScreen`, integración en dashboard, navegación completa. (3) Chat + telemetría: snippet de contexto inyectado en prompts (`focusPromptSnippetService`), sanitización customGoal, modelo `FocusTelemetryEvent`, endpoint `/api/focus/telemetry`, eventos en 8 puntos de interacción. **Tests:** 63 backend (prompt snippet, telemetry, edge, i18n) + 37 frontend. **PR #35** listo para merge. |
| **Crisis 1.5.0+ (#205, #10, #93)** | **Sí* (ingeniería en `main`, merge PR #4 + #5):** camino A/B, `crisisProtocolService`, salida `crisis_protocol_exit`, alertas híbridas (HIGH auto / MEDIUM preguntado + blindaje), T1–T5 en panel, post-envío en UI, biblioteca límites **#194**. Spec: [PROTOCOLO_CRISIS_V1.md](./PROTOCOLO_CRISIS_V1.md). **Revisión clínica/legal aprobada (14 jul 2026).** **Pendiente cierre:** smoke dispositivo físico. |
...
| **Streaming / TTFT (#59)** | **Sí* (jun 2026):** SSE + socket chunks **#128**; métricas TTFT p50/p95 con desglose pre-LLM/modelo; SLO **#67**; `test:streaming-suite` + smoke. Spec smoke: [SMOKE_DISPOSITIVO_STREAMING.md](./SMOKE_DISPOSITIVO_STREAMING.md). **Pendiente cierre:** smoke dispositivo físico; métricas multi-instancia (**#175**). |
...
| **Grounding hechos usuario (#63)** | **Sí* (implementado, pendiente release):** 3 fases completadas — (1) Backend policy + extracción automática: `groundingPolicySnippet.js` (política bilingüe ES/EN anti-alucinación), `userFactsGroundingService.js` (extracción con 18 patrones regex bilingües, sanitización robusta, filtrado contenido sensible con `isValidFactContext` sin `\b`, deduplicación con similarity 0.2, Unicode support `\p{L}`/`\p{N}` para acentos/ñ), integración en `openaiPromptBuilder.js`. (2) Backend CRUD hechos manuales: modelo `UserFact` (validaciones Mongoose + pre-save hook), API REST `/api/user-facts` (POST/GET/PUT/DELETE soft/hard, rate limiting, error handling 400/500), `userFactsApiCopy` i18n ES/EN, schemas Joi custom validators, funciones `getManualUserFacts` + `buildCombinedFactsSnippet` (prioriza manual, deduplica, maxFacts=15). (3) Frontend cliente API: `userFactsService.js` (sanitización `sanitizeText`, validación ObjectId/categorías/sources/límites, manejo de errores con contexto, JSDoc completo). **Tests:** 117 total (81 backend: 48 Fase 1 + 33 Fase 2; 36 frontend Fase 3). **Hardening:** sanitización multicapa (newlines/tabs/espacios/chars problemáticos `<>{}`), validación entrada (userId/conversationId/límites MIN/MAX), filtrado sensible (crisis/abuso/PII), protección legacy data, índices compuestos MongoDB. **PR #36** listo para merge. |
...
| 2 | Focos de acompañamiento | 1–3 temas por temporada (ansiedad, duelo, etc.) que alineen tono, sugerencias y métricas al proceso personal. **Sí* (implementado):** 3 temas (ansiedad, límites, autocuidado), API REST completa, onboarding + progreso UI, integración en dashboard, contexto inyectado en chat prompts, telemetría 8 eventos, hardening completo (sanitización, validación, rate limiting). **63 tests backend + 37 frontend.** Ver estado del código. | Sí* | 3 | 5 | 5 | 5 | M | M | Q2 |
...
| 20 | Compromisos del chat | Lista de “acuerdos” o micro-compromisos detectados o confirmados por el usuario tras la sesión. **Spec (jul 2026):** [CONTRATO_COMPROMISOS_V1.md](./CONTRATO_COMPROMISOS_V1.md). **Base:** API + foco dashboard; pendiente chat, omitir/renegociar, puente tarea/hábito. | Parcial | 2 | 4 | 5 | 5 | M | M | Q2 |
...
| 63 | Grounding hechos usuario | Política: no inventar biografía, fechas ni promesas; solo inferir desde mensajes explícitos; reduce “falso terapeuta que cree saber”. | Sí* | 4 | 5 | 5 | 5 | M | M | Q1 |
...
| 202 | Deberes terapéuticos entre sesiones | La IA asigna **micro-ejercicios** acordados (o confirma los propuestos) y en la **siguiente sesión** pregunta por ellos antes de abrir tema nuevo. **Visión producto (jul 2026):** acompañamiento (instar, no obligar). **Spec:** [CONTRATO_COMPROMISOS_V1.md](./CONTRATO_COMPROMISOS_V1.md). **Base:** `SessionCommitment`, insight, foco; pendiente `proposedCommitments`, follow-up chat, omitir/renegociar, push opt-in. | Parcial | 3 | 4 | 5 | 5 | M | M | Q2 |
...
*Puntuaciones orientativas. Última revisión: **13 jul 2026** (**#2** focos de acompañamiento → **Sí***; crisis **#10**, **#93**, **#205** → **Sí***; **#151** → **Sí***; pendiente común smoke dispositivo; **#9** pospuesto Q4).*
