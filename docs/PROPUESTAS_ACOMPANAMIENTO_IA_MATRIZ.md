...
Listado de **~172** propuestas `No` en la tabla principal, más **~15** `Parcial` / `Sí*` con trabajo abierto — crisis **1.5.x** (**#9**, **#10**, **#205**), señales multimodales (**#215–#217**, **#208**, **#213**), salud digital (**#106**, **#216**), compromisos (**#20**, **#202**), i18n (**#151**, **#153**), límites IA (**#194**) y grafo (**#127**) — release **1.5.1**. **8 anclas entregadas** referenciadas (numeración **#1–#223**; huecos históricos conservados; **16** ex-Q5 + **11** entregadas archivadas). **Hecha:** `Sí` / `Sí*` / `Parcial` = ver columna. Chat → tarea/hábito: [CONTRATO_CHAT_ACCIONES_V1.md](./CONTRATO_CHAT_ACCIONES_V1.md). Bloque salud/patrones: [RELEASE_1.4.4.md](./RELEASE_1.4.4.md).
...
**Anclas entregadas que permanecen en la tabla** (otras propuestas las referencian): **#8** ← #194; **#11** ← #154, #195; **#34** ← #161, #208; **#47** ← #163; **#52–#53** ← #159, #164, #202; **#59** ← #168; **#67** ← #123, #175.

### Estado del código (revisión 26 jun 2026 — v1.5.1)

| Área | Estado |
|------|--------|
| **Release 1.5.1** | Versión app `1.5.1` (iOS build 40, Android `versionCode` 26). Crisis **1.5.0** validada (`validate:release-1.5.0`, `smoke-release-1.5.0.mjs`). Bloque salud/patrones **1.4.4** en prod (`test:release-1.4.4`…`1.4.6`, `test:signals`). |
| **Crisis 1.5.x (#205, #9, #10)** | **Parcial:** **Camino A** hard-stop (`ENABLE_CRISIS_HARD_STOP`): sin LLM ante léxico explícito de ideación/autolesión + riesgo WARNING/MEDIUM/HIGH; respuesta estructurada con recursos por país. **Camino B** blindado: contexto crisis al LLM, constraints MEDIUM, sanitizer post-LLM, extras terapéuticos bloqueados en crisis. **Acciones §3** unificadas (`crisisBackgroundActionsService`) en HTTP y socket. **Panel recursos en chat:** `CrisisResourcesStrip` + `GET /api/health/crisis-resources`. **Ops:** métricas routing, `GET /api/health/crisis-routing`, monitor SLO. **Pendiente producto:** protocolo formal **#93** + revisión legal/clínica; guías para buscar psicólogo (**#9**); UI «por qué» de cada alerta (**#10**); tooltips **#194**. **Deuda ops:** rollups horarios, Redis multi-instancia, dashboard admin, guest sin acciones §3. |
| **Salud digital (#106, #216)** | **Parcial:** consentimiento granular (`SignalConsentPanel`, `/api/signals/consent`). Nativo **HealthKit** (iOS) + **Health Connect** (Android): pasos, sueño, minutos activos; sync multi-día (`digitalHealthSync.js`, `DigitalHealthForegroundSync.js`); `source` confiable en servidor (`healthkit` / `health_connect`). Agregados diarios (`DigitalPhenotypeDailySnapshot`). Snippet chat con gating (`digitalPhenotypeChatContextService`, umbrales, bloqueo en crisis). Card `DigitalHealthStatusCard`; correlaciones en informes. **Alerta de pródromos en Inicio (jun 2026):** `digitalPhenotypePatternAlertService` → `phenotypeAlert` en `/api/summary/focus`; tarjeta `DigitalHealthProdromeAlert` (copy es/en suave no diagnóstico, dismiss por día, a11y). **Blindaje clínico:** se suprime si hay riesgo elevado reciente (72h, mismo criterio `OBSERVATIONAL_BLOCKED_RISK_LEVELS` del chat) y es fail-closed ante errores; `test:signals` (35). **Pendiente:** **Screen Time** nativo (ver sub-item abajo); QA dispositivo completo. |
| **Screen Time nativo (#216 sub-item)** | **No (bloqueo de plataforma):** **iOS** requiere entitlement Apple *Family Controls* + extensión `DeviceActivityReport`; **distribución** necesita aprobación de Apple por bundle ID (días/semanas, hasta 4 formularios) — en dev build funciona con *Family Controls (Development)* pero no sube a TestFlight/App Store sin aprobación. **Android** viable con `@antardev/react-native-usage-stats` (v0.1.x) + permiso `PACKAGE_USAGE_STATS` (el usuario lo concede en Ajustes) + **declaración de permiso en Google Play**. **Plan:** iniciar trámite de aprobación Apple y declaración Play antes de implementar; hoy `screenTimeMinutes`/`socialScreenRatio` degradan a `null`. |
| **Telemetría tecleo (#215)** | **Sí*:** hook `useChatTypingTelemetry` (dwell, flight, backspace, revisiones); opt-in en `SignalConsentPanel`; `POST /api/signals/typing-telemetry`; `cognitiveLoadScore`; alimenta **#217**. `test:signals`. |
| **Motor correlación (#217)** | **Sí*:** `multimodalCorrelationService` cruza tecleo, fenotipo, grafo #127, BA, ABC macro; jobs async vía `weeklyPatternInsightService`. Umbrales conservadores; disclaimers clínicos compartidos. |
| **Informes patrones (#208, #213)** | **Sí*:** informes semanal/mensual (`WeeklyPatternInsight`, `MonthlyPatternInsight`); narrativa heurística + LLM opt-in (`WEEKLY_INSIGHT_LLM_ENABLED`, `weeklyInsightLlmService`). UI revelación: `WeeklyInsightScreen`, `WeeklyInsightHero`, `WeeklyInsightConductCard`. Guardrails en `clinicalContentGuardrails.js`. **Fuera:** informe amplio entre consultas (**#154**). |
| **Home insight rotativo (#34 ext.)** | **Sí*:** `homeRotatingInsightService` + `DashboardRotatingInsightCard` en inicio; insight diario desde patrones semanales/grafo; cache fin de día. |
| **Compromisos (#20, #202)** | **#202 cerrado (Sí*, jun 2026):** `commitmentFollowUpService` retoma el compromiso vencido al iniciar hilo nuevo **o al retomar conversación** desde el foco (`resumeCommitmentFollowUp`); snippet de prompt suave + chips en burbuja (es/en, rehidratables) → `PATCH /api/session-commitments`; cierre por chips **y** detector heurístico de respuesta; `followUpAskedAt` (una vez por compromiso); gating de crisis; paridad SSE/socket; `test:release-1.5.1`. **#20 Parcial:** persistencia + chips en foco; pendiente detección/creación automática desde chat. |
| **Passthrough de snippets de prompt (fix jun 2026)** | **Corregido:** `openaiService` no reenviaba los `*PromptSnippet` a `buildContextualizedPrompt` (stream y no-stream), dejando **muertos** fenotipo (#216), RAG (#203), psicoed, ABC y TCC lite snippet. Helper `applyEnhancementSnippetsToPromptContext` (`ENHANCEMENT_PROMPT_SNIPPET_KEYS`) centraliza el reenvío; test `openaiPromptSnippetPassthrough`. Reactiva esas features en el chat real. |
| **RAG patrones personales (#203)** | **Parcial:** `personalPatternRagService` + vector search topicFree; snippet en prompt si `PERSONAL_PATTERN_RAG_ENABLED` (off por defecto); blindaje contenido sensible. **No** sustituye memoria evolutiva completa ni narrativa «la semana pasada dijiste…». |
...
| **Dashboard foco (#34)** | `DashboardFocusCard` + API agregada; continuidad último hilo, compromisos, racha, mood check-in; insight rotativo (arriba). |
| **i18n UI EN (#151)** | **Parcial:** `LanguageContext`, selector en Ajustes, diccionarios `es`/`en` (~3k claves c/u), wizards TCC (#86–#89) y psicoed (#85) bilingües, FAQ/privacidad EN, `npm run audit:i18n`. **Pendiente:** emails, push, revisión nativa/legal. |
| **Chat multilingüe (#153)** | **Parcial:** `openaiPromptBuilder` responde EN si `language === 'en'`; saludos `chatWelcomeGreeting` + `GREETING_VARIATIONS_EN`; crisis/recursos localizados; `X-App-Language`. **Pendiente:** system prompts versionados por locale en todos los caminos; detección idioma mensaje; PT (**#152**). |
...
| **Grafo tema–intervención (#127)** | **Sí* (padre jun 2026):** pipeline blindado, biblioteca, continuidad topicFree, ranking semántico, backfill embeddings (`test:intervention-graph` 104 + smoke). **En catálogo:** psicoed **#85** + avanzados **#90–#92**; micro-guías **#93–#99**. **Fuera:** #218 producto. |
| **Psicoeducación modular (#85)** | **Sí* (jun 2026):** 12 módulos es/en (9 base + 3 avanzados **#90–#92**), biblioteca, tarjetas chat interactivas (#78), sello #111, `mechanismLine`, smoke + `test:psychoeducation-suite`. |
...
| **WAI post-sesión (#98)** | **Sí* (jun 2026):** formulario 4 ejes 1–5 en `SessionInsight`, umbrales ≥3 turnos/≥80 chars, exclusión crisis/acute/crisis_recovered, skip implícito + recordatorio suave, i18n es/en, métricas + grafo #127 (`session_wai_feedback`), thumbs chat removidos (UI), `test:session-wai-suite` + smoke. **Pendiente:** validación manual dispositivo; **#193** repair fuera MVP. |
...
| Deberes entre sesiones | **Cerrado (Sí*, 1.5.1):** `SessionCommitment` + API + chips en foco + **follow-up en chat** (snippet + chips, disparo en hilo nuevo y al retomar conversación, detector heurístico) | #20, #52–#53 | **#202** |
| Memoria largo plazo / RAG patrones | **Parcial (1.5.1):** `personalPatternRagService` opt-in off; continuidad último hilo (#47); compactación (#60); informes (#208) | #60, #113, #17, #126, #154 | **#203** |
...
| Guardrails crisis (hard-stop + derivación) | **Entregado (jun 2026):** camino A/B, panel recursos, ops routing; pendiente protocolo **#93** y copy ampliado **#9**/**#10** | #9, #10, #93, #194 | **#205** |
...
| Espejo clínico / patrones | **Entregado (1.4.4):** informes semanal/mensual con narrativa LLM + guardrails; home insight rotativo; sin grafo detonantes (#218) | #11, #154, #79 | **#208** |
...
| Tema (tu input) | En la app hoy (pinceladas) | Ya en matriz | Nueva fila |
|-----------------|----------------------------|--------------|------------|
| Dinámica de tecleo (dwell, flight, backspace) | **Entregado (jun 2026):** `useChatTypingTelemetry`, consent opt-in, `POST /api/signals/typing-telemetry`, score carga cognitiva | #135 (señal on-device genérica), #178, #79 | **#215** |
| Fenotipado digital (sueño, pasos, pantalla) | **Parcial (1.5.1):** HealthKit + Health Connect, sync foreground, agregados diarios, snippet chat, informes; sin Screen Time nativo | **#106** (contexto opt-in al modelo), #91, #195 | **#216** |
| Motor correlación multimodal + insight semanal | **Entregado (1.4.4):** `multimodalCorrelationService`, informes semanal/mensual + LLM opt-in, UI **#213** | #203, #208, #213, #154 | **#217** |
...
**Notas de encaje (lote 3):** La UI de “revelación semanal” (**#213**) está entregada con informes **#208**. **#217** es el motor de datos (entregado). La **alerta de pródromos en Inicio** (#216) ya está entregada; lo único que mantiene **#216** en `Parcial` es **Screen Time nativo** (bloqueo de plataforma: aprobación Apple Family Controls + declaración Google Play).
...
| 9 | Derivación humana | Recursos por región: líneas, guías para buscar psicólogo; posiciona la app como puente, no sustituto. **Parcial (jun 2026):** números por país en hard-stop y **panel chat** (`CrisisResourcesStrip` + `GET /api/health/crisis-resources`); preferencia de país en Ajustes; contactos de emergencia en Perfil. **Pendiente:** guías para buscar psicólogo, recursos en dashboard de crisis. | Parcial | 4 | 4 | 2 | 3 | M | M | Q3 |
| 10 | Transparencia crisis | Cuando hay crisis o alertas, explicación clara en lenguaje llano del “por qué” se sugiere una acción. **Parcial (jun 2026):** copy de límites en hard-stop y disclaimers en panel de recursos; alertas push con notificación en chat. **Pendiente:** explicar el «por qué» de cada acción sugerida o alerta en UI dedicada. | Parcial | 4 | 5 | 4 | 5 | B | S | Q1 |
...
| 20 | Compromisos del chat | Lista de “acuerdos” o micro-compromisos detectados o confirmados por el usuario tras la sesión. **Parcial (1.5.1):** modelo `SessionCommitment`, API `/api/session-commitments`, guardar desde `SessionInsightScreen`, chips en foco home, follow-up en chat (**#202**) con cierre de lazo por chips + detector heurístico. **Pendiente:** detección/creación automática desde el chat; lista dedicada. | Parcial | 2 | 4 | 5 | 5 | M | M | Q2 |
...
| 34 | Dashboard del foco | Dashboard del foco: continuidad último hilo, tareas/hábitos, compromisos (#20), racha, mood check-in. **Ext. 1.5.1:** insight rotativo diario (`DashboardRotatingInsightCard`). | Sí* | 3 | 5 | 5 | 5 | M | M | Q1 |
...
| 85 | Psicoeducación modular evidencia | Módulos breves (ansiedad, depresión, ira, sueño, trauma informado) con fuentes y lenguaje no diagnóstico; activables desde chat o biblioteca. **Cerrado (jun 2026):** 12 temas es/en (9 base + **#90–#92**), biblioteca, chat nativo (#78), sello #111, grafo #127. | Sí* | 2 | 4 | 4 | 4 | M | M | Q2 |
...
| 106 | Salud digital contexto opt-in | Lectura opt-in de sueño/pasos (HealthKit / Health Connect) como señales de contexto para el modelo, con granularidad de permisos. **Parcial (1.5.1):** consentimiento granular, sync nativo, snippet en prompt (`digitalPhenotypeChatContextService`, es/en, gating crisis). **Pendiente:** Screen Time; granularidad fina por categoría en producto. | Parcial | 2 | 4 | 4 | 4 | M | L | Q2 |
...
| 151 | Inglés EN producto fase 1 | Cadenas de **UI**, emails transaccionales, errores y copy legal/privacidad en **inglés**; selector de idioma en perfil; pruebas con hablantes nativos o revisión profesional. **En curso:** `LanguageContext`, `translations/en.js` (~3k claves), selector en Ajustes, FAQ EN, privacidad IA bilingüe, `audit:i18n`. **Pendiente:** correos, notificaciones push, revisión nativa/legal completa. | Parcial | 3 | 5 | 5 | 5 | M | L | Q2 |
...
| 153 | Chat IA multilingüe ES/EN/PT | Preferencia de idioma en perfil + **detección opcional** del idioma del mensaje; **system prompts y recursos de crisis** versionados por locale; alinea con **#133** (i18n profundo). **Parcial (1.5.1):** `openaiPromptBuilder` responde EN si `language === 'en'`; saludos `chatWelcomeGreeting`; crisis/recursos localizados; `X-App-Language`. **Pendiente:** prompts versionados en todos los caminos; detección idioma mensaje; PT (**#152**). | Parcial | 3 | 5 | 5 | 5 | M | L | Q2 |
...
| 194 | Biblioteca límites competencia IA | Pantallas y tooltips: **qué no hace** la app, cuándo **#9**, crisis **#10**; reduce expectativas mágicas. **Parcial:** base en #8 (`AIPrivacyScreen` + modal chat); **panel recursos crisis** en chat enlaza derivación (#9). Falta biblioteca ampliada y tooltips contextuales por acción. | Parcial | 4 | 4 | 4 | 4 | M | M | Q1 |
...
| 202 | Deberes terapéuticos entre sesiones | La IA asigna **micro-ejercicios** acordados (o confirma los propuestos) y en la **siguiente sesión** pregunta por ellos antes de abrir tema nuevo. Persistencia ligada a **#20**; puede materializarse como tarea/hábito (**#52–#53**). **Sí* (1.5.1):** `commitmentFollowUpService` — al iniciar hilo nuevo (o al **retomar conversación** desde el foco) con compromiso vencido, snippet de prompt suave (tono sin culpa, atiende primero al usuario, gating de crisis) + chips **lo hice/a medias/aún no** en la burbuja (es/en, rehidratables) que cierran el lazo vía `PATCH /api/session-commitments`; cierre por chips **y** detector heurístico de respuesta en texto; `followUpAskedAt` evita reinsistir; paridad SSE/socket; `test:release-1.5.1`. Persistencia y chips en foco (#20). | Sí* | 3 | 4 | 5 | 5 | M | M | Q2 |
| 203 | Perfil dinámico RAG (patrones) | **Memoria evolutiva** entre sesiones: extraer entidades, emociones y patrones (con consentimiento), índice vectorial por usuario, recuperación en prompt para conectar puntos (“la semana pasada mencionaste…”). Habilita **#210–#212**, **#214**. Distinto de **#113** (corpus curado) y **#60** (solo hilo actual). **Parcial (1.5.1):** `personalPatternRagService` + topicFree vector search; snippet si `PERSONAL_PATTERN_RAG_ENABLED` (off por defecto). **Pendiente:** índice dedicado por usuario, narrativa explícita en producto. | Parcial | 3 | 5 | 5 | 5 | A | L | Q2 |
...
| 205 | Guardrails crisis (hard-stop) | Routing robusto de **riesgo** (ideación, autolesión): **camino A** corta LLM y responde con plantilla + recursos; **camino B** mantiene LLM con blindajes (constraints, sanitizer, sin extras terapéuticos). **Parcial (jun 2026):** `crisisHardStopService`, métricas routing, acciones §3, panel recursos (**#9**), copy límites (**#194**). **Pendiente:** protocolo ideación formal (**#93**), revisión legal/clínica, transparencia ampliada (**#10**). | Parcial | 4 | 5 | 5 | 5 | M | M | Q1 |
...
| 208 | Reporte descubrimiento (espejo clínico) | Cada N sesiones o semanal: **análisis de patrones** (detonantes, resiliencia, temas recurrentes) + visualización ligera; **no** resumen transcripto del chat. **Sí* (1.4.4):** informes semanal/mensual (`weeklyPatternInsightService`), correlaciones #217, narrativa heurística + LLM opt-in (`WEEKLY_INSIGHT_LLM_ENABLED`), guardrails clínicos, home insight rotativo. **Pendiente:** informe amplio entre consultas (**#154**); grafo detonantes (**#218**). | Sí* | 3 | 5 | 5 | 5 | M | L | Q2 |
...
| 213 | UX revelación de insights | Diseño de pantallas de patrón/insight: minimalismo, insight principal en tipografía grande, detalle opcional, gráficos sobrios (neutros, sombras suaves). **Sí* (1.4.4):** `WeeklyInsightScreen`, `WeeklyInsightHero`, `WeeklyInsightConductCard`, disclaimers LLM. Complementa **#204**, **#208–#212**. **Pendiente:** modo refugio global (#204). | Sí* | 3 | 4 | 4 | 4 | M | M | Q2 |
...
| 215 | Telemetría de tecleo (estado cognitivo) | En el input del chat (solo en app, opt-in explícito): **dwell time**, **flight time** y tasa de **backspace** por borrador/mensaje; agregados por sesión sin enviar contenido extra al servidor salvo features resumidas. **Sí* (1.5.1):** `useChatTypingTelemetry`, consent en `SignalConsentPanel`, `POST /api/signals/typing-telemetry`, `cognitiveLoadScore`, alimenta **#217**. `test:signals`. **Fuera:** clasificador on-device (#135). | Sí* | 2 | 4 | 4 | 4 | M | M | Q2 |
| 216 | Fenotipado digital (rutina invisible) | Opt-in **HealthKit / Health Connect** + categorías de **tiempo de pantalla** (APIs nativas): ventana de inactividad (sueño estimado), pasos, ratio social/entretenimiento vs media personal. **Parcial (1.5.1):** collectors iOS/Android (pasos, sueño, minutos activos), sync multi-día + foreground, agregados diarios, `prodromeSleepDelay` en backend, card + informes + snippet chat. **Alerta educativa de pródromos en Inicio entregada** (jun 2026): `DigitalHealthProdromeAlert` + `phenotypeAlert` en `/api/summary/focus`, copy es/en no diagnóstico, dismiss diario. **Pendiente (sub-item Screen Time, bloqueo plataforma):** iOS entitlement Apple Family Controls + aprobación distribución; Android `UsageStatsManager` + declaración Play. Patrón aislamiento completo depende de Screen Time. | Parcial | 3 | 5 | 5 | 5 | M | L | Q2 |
| 217 | Motor correlación multimodal | Cruza señales **#215**, **#216**, extracciones de **#203** y marco clínico (esquemas **#210**, ABC **#212**) en jobs async; produce **insights verificables** (“firma física” ante frustración laboral) para **#208** / sesión semanal con UI **#213**. **Sí* (1.4.4):** `multimodalCorrelationService`, worker `weeklyPatternInsightService`, umbrales conservadores, disclaimers. **Pendiente:** esquemas **#210** en motor; consentimiento granular por fuente en jobs profundos. | Sí* | 3 | 5 | 5 | 5 | A | L | Q2 |
...
*Puntuaciones orientativas. Última revisión: **26 jun 2026** (release **1.5.1**; bloque salud/patrones **1.4.4**; crisis **1.5.0**; **#215–#217**, **#208**, **#213**, **#106**, **#216**, **#20** alineados con código; **#216** alerta de pródromos en Inicio entregada, Screen Time documentado como sub-item con bloqueo de plataforma; **#202** follow-up de compromisos en chat cerrado (Sí*) + fix de passthrough de snippets que reactiva #216/#203/psicoed/ABC en el chat).*
