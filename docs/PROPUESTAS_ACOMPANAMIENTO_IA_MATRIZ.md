...
Listado de **~182** propuestas `No` en la tabla principal, más varias **`Parcial`** con trabajo abierto — entre ellas núcleo **crisis** (**#9**, **#10**, **#205**; release **1.5.0**), i18n (**#151**), límites IA (**#194**) y grafo (**#127**) — y **8 anclas entregadas** referenciadas (numeración **#1–#214**; huecos históricos conservados; **16** ex-Q5 + **11** entregadas archivadas). **Hecha:** `Sí` / `Sí*` / `Parcial` = ver columna. Chat → tarea/hábito: [CONTRATO_CHAT_ACCIONES_V1.md](./CONTRATO_CHAT_ACCIONES_V1.md).
...
### Estado del código (revisión 17 jun 2026)

| Área | Estado |
|------|--------|
| **Crisis 1.5.0 (#205, #9, #10)** | **Parcial (jun 2026):** **Camino A** hard-stop (`ENABLE_CRISIS_HARD_STOP`): sin LLM ante léxico explícito de ideación/autolesión + riesgo WARNING/MEDIUM/HIGH; respuesta estructurada con recursos por país. **Camino B** blindado: contexto crisis al LLM, constraints MEDIUM, sanitizer post-LLM, extras terapéuticos bloqueados en crisis. **Acciones §3** unificadas (`crisisBackgroundActionsService`) en HTTP y socket. **Panel recursos en chat:** `CrisisResourcesStrip` (números marcables, menú ⋮ «Recursos de crisis», rehidratación al refrescar); API pública `GET /api/health/crisis-resources`. **Ops internas:** métricas routing (`crisisRouting` + agregación Mongo), `GET /api/health/crisis-routing`, monitor SLO (`crisisRoutingSloMonitorService`). Smoke/tests release **1.5.0**. **Pendiente producto:** protocolo formal **#93** + revisión legal/clínica; guías para buscar profesional (**#9**); explicación en lenguaje llano del «por qué» en alertas (**#10**); ampliar **#194** con tooltips contextuales. **Deuda ops (próxima versión):** rollups horarios, Redis multi-instancia, dashboard admin, guest sin acciones §3. |
| **RAG patrones personales (#203 pincelada)** | **Opt-in off por defecto:** `personalPatternRagService` + snippet en prompt si `PERSONAL_PATTERN_RAG_ENABLED`; blindaje de contenido sensible. No sustituye **#203** (memoria evolutiva completa). |
...
| **Grafo tema–intervención (#127)** | **Sí* (padre jun 2026):** pipeline blindado, biblioteca, continuidad topicFree, ranking semántico, backfill embeddings (`test:intervention-graph` 104 + smoke). **Fuera del padre:** catálogo #90–#99, #218 producto. |
| **Psicoeducación modular (#85)** | **Sí* (jun 2026):** 9 módulos es/en, biblioteca, tarjetas chat interactivas (#78 microSteps toggle + contrato `psychoeducation_card_v1`), sello #111 en módulo/biblioteca/chat, `mechanismLine`, smoke + `test:psychoeducation-suite`. |
...
| **Jerarquía exposición + SUDS (#87)** | **Parcial (MVP jun 2026):** jerarquía + SUDS, API `/api/exposure-plans`, pantalla `ExposureHierarchy`, catálogo #127, sugerencias ansiedad/evitación, **guardas anti-saltos** + **diálogo de confirmación al avanzar** (#190), **prefill desde chat** (objetivo + 2 pasos sugeridos cuando hay evitación). |
| **Activación conductual (#88)** | **Parcial (~90% jun 2026):** wizard 3 pasos, plan semanal BA, prefill chat, **tendencia de ánimo** en registros recientes, continuidad chat↔BA. **Pendiente:** sync fino tareas↔slots. |
| **Pensamientos automáticos (#89)** | **Parcial (~90% jun 2026):** wizard, picker distorsiones, prefill chat, **borrador AT al cerrar TCC lite** (`POST /tcc-lite-draft`). Integrado con #201. |
| **TCC lite in-chat (#201)** | **Parcial (~90% jun 2026):** 4 pasos, strip progreso, estado en `Conversation.tccLiteState`, handoff AT con borrador, resume desde session insight, snippet LLM protocolos activos (BA/exposición/ABC/AT). **Pendiente:** paridad socket (chat principal usa SSE). |
...
| 85 | Psicoeducación modular evidencia | Módulos breves (ansiedad, depresión, ira, sueño, trauma informado) con fuentes y lenguaje no diagnóstico; activables desde chat o biblioteca. **Cerrado (jun 2026):** 9 temas es/en, biblioteca, chat nativo (#78) con microSteps interactivos + contrato JSON `psychoeducation_card_v1`, sello #111, grafo #127. | Sí* | 2 | 4 | 4 | 4 | M | M | Q2 |
...
| 127 | Grafo ligero tema–intervención | Metadatos estructurados (no NLP pesado) para sugerir módulos #85–#99 y medir qué rutas funcionan. **Padre cerrado (jun 2026):** tracking completo, ranking topicFree + backfill, panel dev, `test:intervention-graph`. **Hijos / futuro:** #90–#99, #218. | Sí* | 2 | 4 | 3 | 4 | M | L | Q2 |
...
| 190 | Escalado exposición sin saltos | Flujo guiado que impide saltar pasos en **#87** sin validación explícita del usuario. **Parcial (jun 2026):** guardas backend (`exposurePlanGuards`) + UI + **diálogo de confirmación** al marcar paso completado (muestra paso actual y siguiente). | Parcial | 3 | 4 | 4 | 4 | M | M | Q2 |
...
| 201 | Marco TCC guiado en chat | Restringir divagación del LLM con **marco clínico TCC**: detectar distorsiones en texto del usuario (catastrofización, todo/nada, etc.), guiar reestructuración **paso a paso** in-chat (no solo consejo genérico). **Parcial (~90% jun 2026):** TCC lite 4 pasos, handoff AT, resume session insight — ver bloque «Estado del código». Disclaimers; revisión clínica (#46, #111). Complementa **#40**, **#89**, **#86**. | Parcial | 3 | 5 | 5 | 5 | M | L | Q2 |
...
*Puntuaciones orientativas. Última revisión: **17 jun 2026** (release **1.5.0** crisis: hard-stop camino A/B, panel recursos chat, ops routing/SLO; **#9**, **#10**, **#205** → Parcial; alineación **#201** con TCC lite; bloque TCC #85–#89 jun 2026; **#201–#223** mayo 2026).*
