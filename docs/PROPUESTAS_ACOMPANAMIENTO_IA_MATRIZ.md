...
| **Streaming / TTFT (#59)** | **Sí* (jun 2026):** SSE `?stream=true` con chunks OpenAI; socket primario + fallback SSE; métricas TTFT p50/p95 en memoria (`/api/metrics/health`) con desglose pre-LLM/modelo; SLO monitor **#67**; suite `test:streaming-suite` + smoke. **Pendiente cierre:** streaming por chunks en socket (**#128**), métricas multi-instancia (**#175**), smoke dispositivo percibido. |
...
| 59 | Streaming / TTFT | Optimizar percepción de velocidad: streaming estable, colas, modelo o ruta más rápida para primera frase; métricas p50/p95 en producción. **Sí* (jun 2026):** SSE streaming, `chatStreamingMetrics` (TTFT + pre-LLM/modelo), `metricsService` p50/p95, SLO **#67**, `test:streaming-suite` + `smoke:streaming`. **Pendiente:** chunks en socket (**#128**), RED multi-instancia (**#175**), smoke dispositivo. | Sí* | 4 | 4 | 4 | 4 | M | S | Q1 |
...
| 168 | Tests unitarios IA streaming | Mocks de OpenAI/SSE, timeouts, cancelación **AbortController**; regresión en **#59**, **#112**, **#128**. **Sí* (jun 2026):** `openaiService.generarRespuestaStream` (off-scope, chunks, circuit breaker), `chatStreamingMetrics`, `chatSseStream` abort, `chatService` signal; suite `test:streaming-suite`. **Pendiente:** paridad socket chunks (**#128**), pipeline unificado (**#112**). | Sí* | 3 | 4 | 3 | 4 | M | M | Q2 |
