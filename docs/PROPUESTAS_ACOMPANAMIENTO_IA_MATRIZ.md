...
Listado de **~182** propuestas `No` en la tabla principal, más **5** filas **`Parcial`** con trabajo abierto — núcleo **crisis** (**#9**, **#10**, **#205**; releases **1.5.0** / **1.5.1**), i18n (**#151**) y límites IA (**#194**) — y **8 anclas entregadas** referenciadas (numeración **#1–#223**; huecos históricos conservados; **16** ex-Q5 + **11** entregadas archivadas). **Hecha:** `Sí` / `Sí*` / `Parcial` = ver columna. Chat → tarea/hábito: [CONTRATO_CHAT_ACCIONES_V1.md](./CONTRATO_CHAT_ACCIONES_V1.md).
...
| **Crisis 1.5.0 (#205, #9, #10)** | **Parcial:** **Camino A** hard-stop (`ENABLE_CRISIS_HARD_STOP`): sin LLM ante léxico explícito de ideación/autolesión + riesgo WARNING/MEDIUM/HIGH; respuesta estructurada con recursos por país. **Camino B** blindado: contexto crisis al LLM, constraints MEDIUM, sanitizer post-LLM, extras terapéuticos bloqueados en crisis. **Acciones §3** unificadas (`crisisBackgroundActionsService`) en HTTP y socket. **Panel recursos en chat:** `CrisisResourcesStrip` (números marcables, menú ⋮ «Recursos de crisis», rehidratación al refrescar); API pública `GET /api/health/crisis-resources`. **Ops:** métricas routing, `GET /api/health/crisis-routing`, monitor SLO. **Post-release (jun 2026):** pipeline de análisis emocional ampliado (anhedonia adolescente, imagen corporal, bullying); mejor respuesta ante pensamientos intrusivos de daño y negación idiomática — refuerzo de calidad, no cierra **#93** ni copy ampliado **#9**/**#10**. **Pendiente:** protocolo formal **#93** + revisión legal/clínica; guías para buscar profesional (**#9**); explicación del «por qué» en alertas (**#10**); tooltips contextuales **#194**. **Deuda ops:** rollups horarios, Redis multi-instancia, dashboard admin, guest sin acciones §3. |
...
| **i18n UI EN (#151)** | **Parcial:** `LanguageContext`, selector en Ajustes, diccionarios `es`/`en` (~2.1k+ claves c/u), wizards TCC y psicoed bilingües. **Avance jun 2026:** campaña email **1.5.0** con copy humano; enlaces Instagram por idioma; recordatorio verificación email. **Pendiente:** push multilingüe, prompts chat/crisis backend (**#153**), revisión nativa/legal completa. |
...
| 9 | Derivación humana | Recursos por región: líneas, guías para buscar psicólogo; posiciona la app como puente, no sustituto. **Parcial (jun 2026):** números por país en hard-stop y **panel chat** (`CrisisResourcesStrip` + `GET /api/health/crisis-resources`); preferencia de país en Ajustes; contactos de emergencia en Perfil; guías psicoed web en antoapps.com (**#85**, no sustituye buscar profesional). **Pendiente:** guías para buscar psicólogo, recursos en dashboard de crisis. | Parcial | 4 | 4 | 2 | 3 | M | M | Q3 |
| 10 | Transparencia crisis | Cuando hay crisis o alertas, explicación clara en lenguaje llano del “por qué” se sugiere una acción. **Parcial (jun 2026):** copy de límites en hard-stop y disclaimers en panel de recursos; alertas push con notificación en chat. **Post-1.5.0:** mejoras de pipeline (pensamientos intrusivos, negación idiomática) no sustituyen UI de «por qué». **Pendiente:** explicar el «por qué» de cada acción sugerida o alerta en UI dedicada. | Parcial | 4 | 5 | 4 | 5 | B | S | Q1 |
...
| 151 | Inglés EN producto fase 1 | Cadenas de **UI**, emails transaccionales, errores y copy legal/privacidad en **inglés**; selector de idioma en perfil; pruebas con hablantes nativos o revisión profesional. **En curso:** `LanguageContext`, `translations/en.js`, selector en Ajustes, FAQ EN, privacidad IA bilingüe, wizards TCC/psicoed. **Avance jun 2026:** campaña email **1.5.0** (copy humano); Instagram por idioma; recordatorio verificación email. **Pendiente:** push multilingüe, revisión nativa/legal completa, cobertura email transaccional total. | Parcial | 3 | 5 | 5 | 5 | M | L | Q2 |
...
*Puntuaciones orientativas. Última revisión: **30 jun 2026** (release **1.5.0** → **1.5.1**; home/hub técnicas/resumen accionable; pulido **#52–#53**; `recurringThemes` en prompt; guías web **#85**; **#191** → Sí*; parciales abiertos: **#9**, **#10**, **#205**, **#151**, **#194**).*
