...
Listado de **~166** propuestas `No` en la tabla principal; **1** fila **`Parcial`** abierta (**#1** ritual inicio). **#225** soft landing → **Sí*** (v1.1 + blindaje). **Smoke dispositivo físico** pendiente en crisis **#10**, **#93**, **#205**, check-in suave **#19**, streaming **#59**/**#128** y validación nativa EN en **#151** (specs [PROTOCOLO_CRISIS_V1.md](./PROTOCOLO_CRISIS_V1.md), [SMOKE_DISPOSITIVO_CRISIS_I18N.md](./SMOKE_DISPOSITIVO_CRISIS_I18N.md), [SMOKE_DISPOSITIVO_STREAMING.md](./SMOKE_DISPOSITIVO_STREAMING.md)). App en tienda/build: **1.5.6** (jul 2026; arco **1.5.0→1.5.6**). **#9** derivación humana **pospuesta** (Q4). Entregas jul 2026 → **Sí***: **#2** focos, **#15** sesiones programadas, **#63** grounding/hechos, **#20**/**#202** compromisos v1, **#203**/**#211** memoria del proceso, **#225** soft landing. **12 anclas entregadas** referenciadas (numeración **#1–#238**; hueco **#231** retirado; **16** ex-Q5 + **11** entregadas archivadas). **Hecha:** `Sí` / `Sí*` / `Parcial` = ver columna. Chat → tarea/hábito: [CONTRATO_CHAT_ACCIONES_V1.md](./CONTRATO_CHAT_ACCIONES_V1.md). Compromisos: [CONTRATO_COMPROMISOS_V1.md](./CONTRATO_COMPROMISOS_V1.md). Patrones: [CONTRATO_PATRONES_EXPERIENCIALES_V1.md](./CONTRATO_PATRONES_EXPERIENCIALES_V1.md). Crisis → [PROTOCOLO_CRISIS_V1.md](./PROTOCOLO_CRISIS_V1.md). Soft landing → [CONTRATO_SOFT_LANDING_POST_CRISIS_V1.md](./CONTRATO_SOFT_LANDING_POST_CRISIS_V1.md). Visión cercana: [VISION_ANTO_LO_QUE_SE_VIENE.md](./VISION_ANTO_LO_QUE_SE_VIENE.md); moonshot: [VISION_ANTO_HORIZONTE.md](./VISION_ANTO_HORIZONTE.md).
...
### Estado del código (revisión 24 jul 2026)

| Área | Estado |
|------|--------|
| **Release app** | **1.5.6** (jul 2026; iOS build 46 / Android 32). Arco **1.5.0→1.5.6:** home unificado, crisis, hub técnicas, resumen accionable, onboarding, paywall, i18n UI EN, focos, hechos, sesiones programadas, compromisos v1, memoria del proceso (#203/#211), check-in de ánimo en home, puente mood→chat, tool `propose_product_action` en prompt, pulido billing Apple/MP. Visión narrativa: [VISION_ANTO_LO_QUE_SE_VIENE.md](./VISION_ANTO_LO_QUE_SE_VIENE.md). |
| **Focos de acompañamiento (#2)** | **Sí* (en `main` / 1.5.5+):** catálogo 3 temas (ansiedad, límites, autocuidado), API `/api/focus/*`, `activeFocus` + `customGoal` editable en home (**#161**), onboarding/progreso UI, snippet en prompts (`focusPromptSnippetService`), telemetría. **Pendiente producto:** retrospectiva de temporada → **#226**. |
| **Sesiones programadas (#15)** | **Sí* (en `main` / 1.5.5+):** API `/api/scheduled-sessions`, UI `ScheduledSessionsScreen`, notificaciones locales Expo + deep link al chat, telemetría `ScheduledSessionEvent`, i18n ES/EN. Spec plan: [PLAN_SESIONES_PROGRAMADAS_15.md](./PLAN_SESIONES_PROGRAMADAS_15.md). |
| **Grounding / hechos (#63)** | **Sí* (en `main` / 1.5.5+):** política anti-alucinación + extracción regex bilingüe, modelo `UserFact`, API `/api/user-facts`, snippet combinado en prompt, UI `UserFactsScreen` (CRUD). Tests backend + frontend. |
| **Compromisos (#20, #202, #234)** | **Sí* v1.1 cerrado (jul 2026):** [CONTRATO_COMPROMISOS_V1.md](./CONTRATO_COMPROMISOS_V1.md) — v1.0 + pantalla «Mis compromisos», «En parte» keep-open con nota, hasta 2 sugerencias en cierre. Blindaje post-crisis 48 h; push semanal opt-in. |
| **Memoria del proceso (#203, #211)** | **Sí* (jul 2026):** [CONTRATO_PATRONES_EXPERIENCIALES_V1.md](./CONTRATO_PATRONES_EXPERIENCIALES_V1.md) — `ExperientialPattern`, consent opt-in, extract al cierre, follow-up «¿ha cambiado un poco?» (#211), recall temático, puente index RAG al extraer; UI Memoria del proceso. RAG reactivo genérico sigue flag (`PERSONAL_PATTERN_RAG_ENABLED`). Smoke: [SMOKE_EXPERIENTIAL_PATTERNS.md](./SMOKE_EXPERIENTIAL_PATTERNS.md). |
| **Check-in ánimo (home)** | **Pincelada (1.5.5+):** `MoodCheckInCard` con CTAs contextuales y puente mood→chat en servidor. Avanza ritual de apertura en home; **no** cierra **#1** (micro-check-in al abrir chat) ni **#159** (check-in 60s mañana). Candidata formalización: **#228**. |
| **Crisis 1.5.0+ (#205, #10, #93)** | **Sí* (ingeniería en `main`):** camino A/B, `crisisProtocolService`, salida `crisis_protocol_exit`, alertas híbridas, T1–T5, biblioteca **#194**. Spec: [PROTOCOLO_CRISIS_V1.md](./PROTOCOLO_CRISIS_V1.md). **Revisión clínica/legal aprobada.** **Pendiente cierre:** smoke dispositivo físico. |
| **Soft landing post-crisis (#225)** | **Sí* (jul 2026, contrato v1.1 + blindaje):** ventana 48 h; mute productivos; strip una vez; home; push alineado; regulation_tap; mute mood→producto; dedupe `entered` y strip con update atómico; ObjectId inválido seguro; rate limit dismiss; reset cliente al borrar chat / nueva ancla. Contrato: [CONTRATO_SOFT_LANDING_POST_CRISIS_V1.md](./CONTRATO_SOFT_LANDING_POST_CRISIS_V1.md). Evolución natural: reentrada productiva **#229**. |
| **Continuidad chat (#4, #47)** | Worker `lastSessionSummary`, tarjeta perfil, bloque foco en inicio (`GET /api/summary/focus`). Compromiso y continuidad no se duplican en preview. |
| **Acciones desde chat (#52, #53)** | Contrato v1 + tool `propose_product_action` (elegibilidad servidor, máx. 1/turno, default no llamar). Cap/cooldown, persistencia al reabrir hilo. **#116** sigue **No** (suite amplia de tools: listar hábitos, escalas, etc.). |
| **Dashboard foco (#34)** | Home: `DashboardFocusCard` + insight + compromisos + continuidad; objetivo editable (**#161**); check-in ánimo colapsable. |
| **Resumen / informe (#11, #154)** | **#11** accionable. Informe observacional pulido (estados calmados). **#154**/**#208** siguen **No** (espejo de patrones / informe ampliado). |
| **Hub de técnicas** | Tab «Técnicas»; Pomodoro al enfocar tarea (**#54** sigue **No**). |
| **Onboarding / bienvenida (#16)** | Archivada; 1.5.x con orb, sesión persistente, bootstrap estable. Journey primera semana → **#228**. |
| **i18n UI EN (#151)** | **Sí*:** paridad CI es/en, selector Ajustes, wizards, emails, push. **Pendiente:** revisión nativa EN + smoke dispositivo. Chat multilingüe → **#153**. |
| **Streaming / TTFT (#59, #128)** | **Sí*:** SSE + socket chunks, métricas TTFT, SLO **#67**. **Pendiente:** smoke dispositivo; RED multi-instancia (**#175**). |
| **Panel IA (#8) / límites (#194)** | **Sí:** `AIPrivacyScreen` + biblioteca límites. |
| **CI lint unificado (#167)** | Sigue **No**: CI frontend `check` + Jest; ESLint sin gate. |
| **Grafo / psicoed / TCC / WAI** | Sin cambio de alcance vs jun 2026: **#85–#92**, **#78**, **#86–#89**, **#190–#191**, **#98**, **#127**, **#201**, **#212** en **Sí*** (suites + smokes canónicos). |
| **Saludo inicial chat (i18n)** | **Parcial:** variaciones EN + `X-App-Language`; welcome localizado en cliente. |
| **Paywall / billing** | Memoria del día en paywall; hardening Apple IAP / MP en curso (1.5.6). |

### Nuevas propuestas (input producto)

Incorporadas como **#201–#223** (lotes 1–3, mayo 2026), **#224–#228** (lote 4), **#229–#233** (lote 5; **#231** retirada) y **#234–#238** (lote 6, jul 2026). Criterio: no duplicar filas ya cubiertas; donde hay **pinceladas en app**, la fila nueva describe el salto de producto pendiente y enlaza la fila existente. Filtro narrativa: [PRODUCT_NARRATIVE.md](./PRODUCT_NARRATIVE.md) (≥3 sí; acompañamiento y empoderamiento obligatorios).
...
| Deberes entre sesiones | Tareas/hábitos desde chat (#52–#53); compromisos (#20) con follow-up parcial en dashboard | #20, #52–#53 | **#202** → [CONTRATO_COMPROMISOS_V1.md](./CONTRATO_COMPROMISOS_V1.md) |
...
**Lote 4 (#224–#228) — cierre del ciclo 1.5.x / puente a 1.6 (jul 2026)**

Tras focos, hechos, sesiones programadas, compromisos v1 y memoria del proceso, el salto cercano es **presencia + cierre del arco de temporada** (visión 1.5.8→1.6), sin abrir multimodal (#215–#217).

| Tema | En la app hoy (pinceladas) | Ya en matriz | Nueva fila |
|------|----------------------------|--------------|------------|
| Frase propia al cierre («me llevo…») | Continuidad #47 es texto IA; WAI #98 es escala; sin línea escrita por el usuario | #4, #47, #98, #164 | **#224** |
| Soft landing post-crisis | **Sí* v1.1 + blindaje** (ventana 48 h, mute, strip, home, push) | #19, #93, #205, #10 | **#225** |
| Retrospectiva de temporada de foco | Foco activo + `customGoal` (#2, #161); sin cierre de temporada ni “qué cambió” | #2, #161, #208, #49 | **#226** |
| Panel memoria unificado | `UserFactsScreen` (#63) y Memoria del proceso (#203) separados; consentimientos dispersos | #63, #203, #33 | **#227** |
| Primera semana acompañada | Onboarding (#16 arch.) + check-in ánimo home; sin journey D1–D7 guiado | #16, #1, #159, #2, #15 | **#228** |

**Lote 5 (#229–#233) — post soft landing / puente presencia 1.5.8 (jul 2026)**

Con **#225** cerrado: reentrada, ritual al abrir chat, contactos y modo presencia. **#231** (pausa favorita) **retirada** (jul 2026) — hueco conservado.

| Tema | En la app hoy (pinceladas) | Ya en matriz | Nueva fila |
|------|----------------------------|--------------|------------|
| Reentrada productiva post ventana 48 h | Soft landing mutea CTAs; al vencer la ventana vuelven a pleno sin rampa | #225, #202, #52–#53, #164 | **#229** |
| Ritual de llegada al chat | Check-in ánimo en home; follow-up #211; **#1** sigue Parcial (sin micro-línea al abrir chat) | #1, #159, #228 | **#230** |
| Aviso «estoy bien» a contactos | Alertas de crisis / contactos de emergencia; sin mensaje breve iniciado por el usuario al estabilizarse | #10, #9, #225 | **#232** |
| Modo solo presencia (24–48 h) | Soft landing es automático post-crisis; `responseStyle` fijo; sin interruptor temporal «solo presencia» | #225, #3, #209, #36 | **#233** |

**Lote 6 (#234–#238) — cierres de arco visibles / win del día (jul 2026)**

Ángulo distinto al lote 5: **cerrar deuda de producto ya a medias** y **continuidad tangible en home/chat**, sin modos post-crisis ni multimodal.

| Tema | En la app hoy (pinceladas) | Ya en matriz | Nueva fila |
|------|----------------------------|--------------|------------|
| Compromisos v1.1 (lista + «En parte») | v1.0 en chat/home; contrato marca v1.1 pendiente sin fila propia | #20, #202 | **#234** |
| Handoff técnica → chat | Hub/técnicas abren pantalla; al volver el hilo no sabe que hubo pausa | #78, #52, #47 | **#235** |
| Título de conversación sugerido | Lista de hilos genérica / sin título útil; continuidad no nombra el tema | #47, #60 | **#236** |
| Chip semanal de foco | Foco activo (#2); sin check «¿seguimos?» ni cambio ligero | #2, #161, #226 | **#237** |
| Insight de una línea en home | Patrones (#203/#211) y resumen (#11); home sin frase observacional breve | #203, #208, #34 | **#238** |
...
|---|-----------|-------------|-------|-----|-----|-----|------|-------|---|---|
| 1 | Ritual inicio sesión | Al abrir el chat: micro-check-in o una línea de continuidad (“cómo llegas hoy”, “lo último que quedó pendiente”) antes de la lista fría de conversaciones. **Parcial (jul 2026):** follow-up experiencial (#211) al primer turno / resume desde home cuando hay patrón due; check-in de ánimo en home + puente mood→chat (pincelada). **No cubre** micro-check-in genérico al abrir el chat. Cierre propuesto: **#230**; journey D1–D7: **#228**. | Parcial | 3 | 4 | 4 | 4 | M | S | Q2 |
| 2 | Focos de acompañamiento | 1–3 temas por temporada (ansiedad, duelo, etc.) que alineen tono, sugerencias y métricas al proceso personal. **Sí* (1.5.5+):** 3 temas (ansiedad, límites, autocuidado), API `/api/focus/*`, onboarding + progreso, dashboard, snippet en prompts, telemetría, `customGoal` editable (#161). Evolución: retrospectiva de temporada **#226**. | Sí* | 3 | 5 | 5 | 5 | M | M | Q2 |
...
| 15 | Sesiones programadas | Recordatorios tipo “tu espacio con Anto” en horarios fijos para ritualizar el encuentro. **Sí* (1.5.5+):** API + `ScheduledSessionsScreen`, notificaciones locales, deep link al chat, telemetría de adherencia, i18n ES/EN. | Sí* | 3 | 4 | 5 | 5 | B | S | Q2 |
...
| 20 | Compromisos del chat | Lista de “acuerdos” o micro-compromisos detectados o confirmados por el usuario tras la sesión. **Sí* v1.0 (jul 2026):** [CONTRATO_COMPROMISOS_V1.md](./CONTRATO_COMPROMISOS_V1.md) — propuestas chat, follow-up unificado, omitir/renegociar, puente tarea/hábito, post-crisis guard. Mantra: «Lo que quedó para retomar». **v1.1:** pantalla dedicada y pulidos menores. Pareja producto: **#202**. | Sí* | 2 | 4 | 5 | 5 | M | M | Q2 |
...
| 55 | Paráfrasis + validación antes consejo | Regla de pipeline/prompt: reflejar con palabras del usuario el sentimiento o necesidad **antes** de orientar o intervenir (micro-habilidad terapéutica). | Sí* | 3 | 5 | 5 | 5 | M | M | Q2 |
...
| 63 | Grounding hechos usuario | Política: no inventar biografía, fechas ni promesas; solo inferir desde mensajes explícitos; reduce “falso terapeuta que cree saber”. **Sí* (1.5.5+):** policy + extracción + `UserFact` CRUD API + `UserFactsScreen` + snippet en prompt. Unificación UI con patrones → **#227**. | Sí* | 4 | 5 | 5 | 5 | M | M | Q1 |
...
| 128 | Streaming Socket paridad SSE | Mismo streaming por chunks que SSE en HTTP; UX y telemetría coherentes con **#76**. **Sí* (jun 2026):** `message:chunk`, TTFT socket, filtro `conversationId`, chunk sintético si no hay deltas, cancelación turno, test paridad. Smoke: [SMOKE_DISPOSITIVO_STREAMING.md](./SMOKE_DISPOSITIVO_STREAMING.md). **Pendiente:** smoke dispositivo físico. | Sí* | 3 | 4 | 4 | 4 | M | M | Q2 |
...
| 161 | Objetivo semanal en home | Texto corto **escrito por el usuario** (o confirmado) visible en dashboard; refuerza **#34** y **#20** con intención explícita. **Sí* (jul 2026):** edición inline de `activeFocus.customGoal` en home (`DashboardFocusCard` + `PATCH /api/focus/active`); empty CTA; sin modelo semanal independiente. | Sí* | 3 | 4 | 4 | 4 | B | S | Q2 |
...
| 202 | Compromisos entre sesiones (follow-up) | Acuerdos entre conversaciones con follow-up suave (no «deberes»). **Sí* v1.0 + v1.1 (#234, jul 2026):** [CONTRATO_COMPROMISOS_V1.md](./CONTRATO_COMPROMISOS_V1.md) — chips, renegociación, puente #52–#53, push opt-in, pantalla lista, En parte keep-open, 2 sugerencias en cierre. Pareja: **#20**, **#234**. | Sí* | 3 | 4 | 5 | 5 | M | M | Q2 |
| 203 | Perfil dinámico RAG (patrones) | **Memoria evolutiva** entre sesiones: extraer entidades, emociones y patrones (con consentimiento), índice vectorial por usuario, recuperación en prompt para conectar puntos (“la semana pasada mencionaste…”). **Sí* (jul 2026):** `ExperientialPattern` + consent + extract worker al cierre + follow-up proactivo (#211) + puente index RAG al extraer; UI Memoria del proceso; contrato `CONTRATO_PATRONES_EXPERIENCIALES_V1.md`. RAG reactivo genérico sigue opt-in (`PERSONAL_PATTERN_RAG_ENABLED`). Distinto de **#113** / **#60**. | Sí* | 3 | 5 | 5 | 5 | A | L | Q2 |
...
| 205 | Guardrails crisis (hard-stop) | Routing de riesgo: camino A (plantilla) + camino B (LLM blindado). **Sí* (jun 2026):** hard-stop, protocolo v1, MEDIUM preguntado con validación, T1–T5, `crisis_protocol_exit` (merge PR #4). **Revisión clínica/legal aprobada (14 jul 2026).** **Pendiente:** smoke dispositivo físico. | Sí* | 4 | 5 | 5 | 5 | M | M | Q1 |
...
| 211 | Espejo puntos ciegos (disonancia) | Si el relato actual **contradice** valores o conductas reportadas antes, reflejo **neutral y delicado** con invitación a explorar el cambio de percepción. Depende de memoria entre sesiones (**#203**); no confrontación. Complementa **#61**, **#208**. **Sí* (jul 2026):** follow-up evolutivo «¿ha cambiado un poco?» con ancla temporal + chips changed/unchanged/skipped; coexistencia con #202 (compromiso gana). No cubre contraste automático con valores/conductas fuera de patrones experienciales. | Sí* | 3 | 5 | 5 | 5 | M | L | Q2 |
...
| 224 | Frase propia al cierre | Al cerrar sesión (o tras insight), el usuario escribe **una línea** «me llevo…» en su voz; se guarda junto a la continuidad IA (#47) y puede reaparecer en home/perfil. Empodera el cierre; no sustituye bullets generados. Complementa **#4**, **#98**, **#164**. | No | 3 | 4 | 5 | 5 | B | S | Q2 |
| 225 | Soft landing post-crisis | Ventana **48 h** tras `crisis_protocol_exit` o `crisis_hard_stop`: tono breve/presente, CTAs productivos apagados (salvo chat + regulación + recursos), strip una vez, home calmado, push follow-up alineado. Spec: [CONTRATO_SOFT_LANDING_POST_CRISIS_V1.md](./CONTRATO_SOFT_LANDING_POST_CRISIS_V1.md) **v1.1**. Distinto de **#19**. **Sí* (jul 2026):** v1 + v1.1 + blindaje (ack atómico entered/strip, rate limit dismiss, ObjectId seguro, reset cliente). Evolución: **#229**. Pendiente opcional: smoke dispositivo. | Sí* | 4 | 5 | 4 | 5 | M | M | Q1 |
| 226 | Retrospectiva temporada de foco | Al cerrar o renovar un foco (#2): pantalla breve «qué cambió en esta temporada» (3 prompts + opción omitir), sin diagnóstico; puede alimentar insight semanal (#208 lite). Escala 0–10 opcional (#49). Cierre narrativo del arco de foco. | No | 3 | 5 | 5 | 5 | M | M | Q2 |
| 227 | Panel memoria unificado | Una sola superficie en Ajustes/Perfil: hechos (#63), patrones experienciales (#203), consentimientos y borrado. Reduce silos y refuerza control del usuario. Complementa **#33** (consentimientos granulares) sin esperar su alcance completo. | No | 3 | 4 | 4 | 4 | M | M | Q2 |
| 228 | Primera semana acompañada | Journey suave D1–D7 post-onboarding: elegir foco (#2), un check-in, una sesión programada (#15), primer compromiso opcional (#202) — todo omitible. Une pinceladas de ánimo en home y ritual **#1**/**#159** en un arco de retención temprana sin culpa. | No | 3 | 4 | 5 | 5 | M | M | Q2 |
| 229 | Reentrada productiva suave | Al **vencer** la ventana soft landing (#225), no restaurar de golpe todos los CTAs: primera oferta productiva (compromiso / tarea / hábito) es **una sola**, con copy «cuando quieras» y omitir fácil; rampa 24 h opcional. Evita cliff de presión post-crisis. Complementa **#164**, **#202**. | No | 4 | 4 | 5 | 5 | B | S | Q1 |
| 230 | Ritual de llegada al chat | Al abrir el chat (no solo home): micro-línea «¿cómo llegas?» o continuidad breve antes de la lista fría — omitible, 1 vez por visita. Cierra el hueco **Parcial** de **#1** sin esperar el journey D1–D7 (**#228**). Distinto de check-in ánimo home y de **#159** (mañana fija). | No | 3 | 4 | 5 | 5 | B | S | Q2 |
| 232 | Aviso «estoy bien» a contactos | Tras estabilizarse (post soft landing o salida de protocolo), el usuario puede **enviar un mensaje breve** a contactos de emergencia («estoy bien / gracias por estar») — iniciado por él, editable, sin auto-envío. Empodera el cierre del arco de crisis sin derivación humana (**#9**). Complementa **#10**. | No | 3 | 4 | 4 | 4 | M | M | Q2 |
| 233 | Modo solo presencia | Interruptor temporal (24–48 h) o sugerencia tras ánimo bajo: Anto prioriza validación y presencia; mutea propuestas productivas y psicoed — **sin** exigir crisis previa. Distinto de soft landing automático (#225) y de modos permanentes (#209). Usuario activa/desactiva. | No | 3 | 5 | 5 | 5 | M | M | Q2 |
| 234 | Compromisos v1.1 (lista + En parte) | Pantalla **«Mis compromisos»**, **«En parte»** keep-open + `partialNote`, hasta **2** `suggestedCommitments` en cierre. Cierra deuda de **#202** v1.0. Spec: [CONTRATO_COMPROMISOS_V1.md](./CONTRATO_COMPROMISOS_V1.md) **v1.1**. **Sí* (jul 2026).** | Sí* | 3 | 5 | 5 | 5 | M | M | Q2 |
| 235 | Handoff técnica → chat | Al volver de respiración/grounding (u otra técnica nativa) al chat: línea de contexto en el hilo («volví de una pausa») y Anto retoma con tono de continuidad, sin reiniciar frío ni proponer tarea. Complementa **#78**, **#47**. | No | 3 | 4 | 4 | 4 | B | S | Q2 |
| 236 | Título de conversación sugerido | Tras 2–3 turnos con sustancia, Anto **propone un título** editable (confirmación explícita); mejora la lista de hilos y el reconocimiento al retomar. Opt-out; sin auto-renombrar. | No | 2 | 4 | 4 | 4 | B | S | Q2 |
| 237 | Chip semanal de foco | Una vez por semana (o al abrir home tras 7 días con foco): «¿seguimos con [foco]?» — Sí / Cambiar / Omitir. Renueva temporada sin la retrospectiva completa (**#226**). Refuerza **#2**. | No | 3 | 4 | 5 | 5 | B | S | Q2 |
| 238 | Insight de una línea en home | En el bloque de foco/home: **una frase observacional** omitible desde patrón experiencial due o tema de foco («esta semana volvió el tema de…»), no informe ni gráfico. Lite de **#208**; no sustituye **#211**. | No | 3 | 4 | 5 | 5 | M | S | Q2 |

---

*Puntuaciones orientativas. Última revisión: **24 jul 2026** (app **1.5.6**; **#225** y **#234** (compromisos v1.1) → **Sí***; **#231** retirada; lote **#235–#238** abiertos; **#1** Parcial; smoke dispositivo crisis/streaming/i18n EN pendiente; **#9** Q4).*
