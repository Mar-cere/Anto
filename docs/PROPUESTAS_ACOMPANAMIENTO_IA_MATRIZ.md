...
| **Crisis 1.5.0+ (#205, #9, #10, #93)** | **Parcial (ingeniería en `main`, merge PR #4):** camino A/B, `crisisProtocolService`, salida `crisis_protocol_exit`, alertas híbridas (HIGH auto / MEDIUM preguntado + blindaje), T1–T5 en panel, post-envío en UI. Spec: [PROTOCOLO_CRISIS_V1.md](./PROTOCOLO_CRISIS_V1.md). **Pendiente cierre producto:** firma clínica/legal, smoke dispositivo, tests integración (`crisisDetectionFlow`). |
...
| 10 | Transparencia crisis | Explicación en lenguaje llano del «por qué» en modo crisis. **En `main`:** bloques T1–T5 en `CrisisResourcesStrip` (`crisisProtocolCopy` es/en); T5 si hubo alerta; aviso post-envío §5.3. **Pendiente cierre:** firma clínica/legal, smoke dispositivo, revisión copy EN nativa (#151). | Parcial | 4 | 5 | 4 | 5 | B | S | Q1 |
...
| 93 | Protocolo ideación (seguridad) | Flujo fijo: validación, seguridad, recursos, acompañamiento, alertas híbridas. **En `main`:** [PROTOCOLO_CRISIS_V1.md](./PROTOCOLO_CRISIS_V1.md) + `crisisProtocolService` (batería, salida 2 turnos / mensaje explícito, estado en conversación). **Pendiente cierre:** firma clínica/legal, smoke, tests integración. | Parcial | 4 | 5 | 5 | 5 | A | M | Q1 |
...
| 205 | Guardrails crisis (hard-stop) | Routing de riesgo: camino A (plantilla) + camino B (LLM blindado). **En `main`:** hard-stop, protocolo v1, MEDIUM preguntado con validación, T1–T5, `crisis_protocol_exit` (merge PR #4). **Pendiente cierre:** firma clínica/legal de #93, smoke dispositivo. | Parcial | 4 | 5 | 5 | 5 | M | M | Q1 |
...
*Puntuaciones orientativas. Última revisión: **30 jun 2026** (merge PR #4 en `main`; Q1 parciales abiertos: **#10**, **#93**, **#194**, **#205**; fuera Q1: **#9** Q3, **#151** Q2).*
