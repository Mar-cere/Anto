...
| **Autorregistro ABC (#86)** | **Sí* (jun 2026):** wizard A→B→C, API, export, pantalla `AbcRecord`, catálogo #127, sugerencias chat, prefill A+B desde mensaje (es/en), ciclo macro en pantalla (#212), `test:abc-suite` + smoke dispositivo. |
...
| 212 | Ciclo ABC macro visual | Agrupar eventos similares en **Detonante → Pensamiento → Conducta → Consecuencia**; lienzo interactivo para ver dónde intervenir. **Parcial (jun 2026):** agregación + API `detail=cycle` + visual A→B→C en `AbcRecord`/Resumen/Estadísticas/informe; continuidad chat factual. **Pendiente:** lienzo interactivo (#207). Distinto de ABC puntual (**#86**). Acopla **#207**, **#208**. | Parcial | 3 | 5 | 4 | 5 | M | L | Q2 |
