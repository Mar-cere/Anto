...
describe('release 1.5.x guard', () => {
  it('frontend alinea versión 1.5.3 y builds de tienda mínimos', () => {
    const app = JSON.parse(readRepo('frontend/app.json'));
    const pkg = JSON.parse(readRepo('frontend/package.json'));
    expect(app.expo.version).toBe('1.5.3');
    expect(pkg.version).toBe('1.5.3');
    expect(Number.parseInt(app.expo.ios.buildNumber, 10)).toBeGreaterThanOrEqual(43);
    expect(app.expo.android.versionCode).toBeGreaterThanOrEqual(29);
  });

  it('backend expone APP_VERSION 1.5.3 en health y package', () => {
    const server = readRepo('backend/server.js');
    const pkg = JSON.parse(readRepo('backend/package.json'));
    expect(server).toMatch(/const APP_VERSION = '1\.5\.3'/);
    expect(pkg.version).toBe('1.5.3');
  });
