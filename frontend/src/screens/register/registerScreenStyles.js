/**
 * Estilos para RegisterScreen y subcomponentes (derivados del tema activo).
 */
...
const modalPrimaryTintAlpha = (hex) => {
  const h = hex.replace('#', '');
...
export function createRegisterScreenStyles(colors) {
...
export function useRegisterScreenStyles() {
  const { colors } = useTheme();
  return useMemo(() => createRegisterScreenStyles(colors), [colors]);
}
